...
export class CognitoWebNativeConstruct extends Construct {
  ...
  constructor(
    ...
    /* define the autonomous communities of Spain */
    const communities: string[] = ["Andalucia", "Aragon", "Canarias", "Cantabria", "CastillaYLeon", "CastillaLaMancha", "Cataluna", "Ceuta", "ComunidadValenciana", "ComunidadDeMadrid", "Extremadura", "Galicia", "IslasBaleares", "LaRioja", "Melilla", "Navarra", "PaisVasco", "PrincipadoDeAsturias", "RegionDeMurcia"]
    
    /* create a user group for each community */
    communities.forEach( (element) => {
      new cognito.CfnUserPoolGroup(this, element.concat('PoolGroup'), {
      userPoolId: userPool.userPoolId,
      groupName: element
      })
    });
